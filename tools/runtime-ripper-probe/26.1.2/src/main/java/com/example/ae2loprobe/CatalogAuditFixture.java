...
public final class CatalogAuditFixture {
    ...
    public static void begin(ServerLevel serverLevel, IGrid activeGrid, CraftingRipperBlockEntity block, Path evidence) {
        ...
        try {
            ...
            var recipes = new ArrayList<RecipeHolder<?>>(level.getServer().getRecipeManager().getRecipes());
            recipes.sort(Comparator.comparing(h -> h.id().identifier().toString()));
    ...
    public static void tick() {
        ...
        try {
            if (stage == 1) {
                ...
                var persistence = CatalogPatternPersistenceFixture.run(level, ripper, networkPatterns());
                report.put("catalogPatternPersistence", persistence);
                report.put("catalogPatternPersistencePassed", persistence.get("passed"));
                passed = lifecyclePassed && ((Number) report.get("missingPublishableRecipeCount")).intValue() == 0
                        ...
                        && Boolean.TRUE.equals(report.get("catalogPatternPersistencePassed"));
                finished = true;
    ...
    public static Map<String, Object> summary() {
        ...
        for (String key : List.of("passed", "actualPatternCount", "publishableRecipeCount", "missingPublishableRecipeCount",
                ...
                "catalogPatternPersistencePassed",
    ...
    private static void inspect(RecipeHolder<?> holder) {
        ...
        var row = new Row(holder.id().identifier().toString(), type, recipe.getClass().getName(), recipe.isSpecial());
        rows.put(row.id, row);
        ...
        try {
            if (recipe instanceof CraftingRecipe crafting) {
                ...
                for (var ingredient : alternatives(stonecutting.input())) {
                    var input = new SingleRecipeInput(ingredient);
                    if (!stonecutting.matches(input, level)) continue;
                    var assembled = stonecutting.assemble(input);
                ...
                var templates = smithing.templateIngredient().map(CatalogAuditFixture::alternatives).orElse(List.of(ItemStack.EMPTY));
                var bases = alternatives(smithing.baseIngredient());
                var additions = smithing.additionIngredient().map(CatalogAuditFixture::alternatives).orElse(List.of(ItemStack.EMPTY));
                for (var template : templates) for (var base : bases) for (var addition : additions) {
                    ...
                    var assembled = smithing.assemble(input);
                    if (assembled.isEmpty()) continue;
                    row.nativeMatches++;
                    if (template.isEmpty() || addition.isEmpty()) {
                        optionalSmithingWitness(row, holder, input, assembled);
                        continue;
                    }
        ...
        if (row.witnesses.isEmpty()) row.reasons.add("no_encodable_recipe_witness_found_not_proof_of_impossibility");
    }

    private static void optionalSmithingWitness(Row row, RecipeHolder<?> holder,
            SmithingRecipeInput input, ItemStack assembled) {
        try {
            var nativeEncoded = PatternDetailsHelper.encodeSmithingTablePattern(cast(holder),
                    AEItemKey.of(input.template()), AEItemKey.of(input.base()), AEItemKey.of(input.addition()),
                    AEItemKey.of(assembled), false);
            accept(row, "native_optional_smithing", nativeEncoded, assembled,
                    List.of(input.template(), input.base(), input.addition()));
            return;
        } catch (RuntimeException nativeRejected) {
            row.reasons.add("native_encoder_rejected_optional_smithing:" + nativeRejected.getClass().getSimpleName());
        }
        // The witness output above comes from the registered vanilla recipe. Custom decoding is a separate claim.
        var custom = CatalogSmithingPattern.fromRecipe(holder, input, level);
        accept(row, "custom_optional_smithing_true_empty_slots", custom.getDefinition().toStack(), assembled,
                List.of(input.template(), input.base(), input.addition()));
    }
    ...
    private static void inspectCrafting(Row row, RecipeHolder<CraftingRecipe> holder, CraftingRecipe recipe) {
        ...
        for (int index : placement.slotsToIngredientIndex()) {
            ingredients.add(index < 0 ? null : placement.ingredients().get(index));
        }
    ...
    private static void craftingWitness(Row row, RecipeHolder<CraftingRecipe> holder, CraftingRecipe recipe,
        ...
        try {
            ...
            if (!ItemStack.matches(assembled, recipe.assemble(input))) {
    ...
    private static void accept(Row row, String fixture, ItemStack encoded, ItemStack expected, List<ItemStack> inputs) {
        ...
        if (pattern == null) { row.reasons.add("registered_pattern_decoder_rejected:" + fixture); return; }
        if (!row.id.equals(recipeId(pattern)) || pattern.getPrimaryOutput().amount() != expected.getCount()
            ...
            row.reasons.add("pattern_round_trip_output_or_recipe_mismatch:" + fixture);
            return;
        }
    ...
    private static void auditPublished(Set<IPatternDetails> actual) {
        ...
        for (var row : rows.values()) {
            ...
            publishable++;
            if (row.witnesses.stream().anyMatch(w -> !(w.pattern instanceof CatalogSmithingPattern))) {
                counts.merge("nativeWitnessPublishableRecipeIds", 1, Integer::sum);
            }
            if (row.witnesses.stream().anyMatch(w -> w.pattern instanceof CatalogSmithingPattern)) {
                counts.merge("customWitnessPublishableRecipeIds", 1, Integer::sum);
            }
    ...
    private static Map<String, Object> inspectPublishedExecution(Row row, Witness witness) {
        ...
        var holder = level.getServer().getRecipeManager().byKey(net.minecraft.resources.ResourceKey.create(Registries.RECIPE, net.minecraft.resources.Identifier.parse(row.id))).orElse(null);
        if (changed && holder != null && holder.value() instanceof CraftingRecipe recipe) {
            ...
            negativeApplicable = recipe.matches(nativeInput, level)
                    && !ItemStack.matches(witness.expectedOutput, recipe.assemble(nativeInput));
        }
    ...
    private static String recipeId(IPatternDetails pattern) {
        if (pattern instanceof com.example.ae2lightoptimizer.integration.CatalogSmithingPattern smithing) {
            return smithing.recipeId();
        }
        ...
        return stonecutting == null ? "<non-native-pattern>" : stonecutting.recipeId().identifier().toString();
    }
    ...
    private static void buildUpgradeFacadeAndBannerFrames() {
        ...
        for (var holder : level.getServer().getRecipeManager().getRecipes()) {
            if (holder.value() instanceof StorageCellUpgradeRecipe upgradeRecipe) {
                frame("native_storage_cell_upgrade_" + holder.id().identifier().toString(),
    ...
    private static List<ItemStack> alternatives(Ingredient ingredient) { return ingredient == null ? List.of() : ingredient.items().map(h -> h.value().getDefaultInstance()).toList(); }
    ...
    private static final class Witness {
        ...
        Map<String, Object> describe() { return Map.of("fixture", fixture, "inputs", inputs, "result", result,
                ...
                "nativeEncodable", !(pattern instanceof CatalogSmithingPattern),
                "customVerified", pattern instanceof CatalogSmithingPattern); }
    }
    private static final class Row {
        ...
        Map<String, Object> describe() {
            ...
            result.put("nativeMatchingExamples", nativeMatches);
            result.put("publishableByNativeWitness", witnesses.stream().anyMatch(w -> !(w.pattern instanceof CatalogSmithingPattern)));
            result.put("publishableByCustomWitness", witnesses.stream().anyMatch(w -> w.pattern instanceof CatalogSmithingPattern));
