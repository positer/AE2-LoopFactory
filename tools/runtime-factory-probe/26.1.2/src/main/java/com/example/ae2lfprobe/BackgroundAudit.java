...
public final class BackgroundAudit {
    ...
    private static final Set<String> checkedGuideLayouts = new HashSet<>();
    private static volatile boolean minimalNetworkRan;
    private static volatile String minimalNetworkDebug="";
    private static void verifyGuideLayout(guideme.internal.screen.GuideScreen screen, String locale) {
        if(checkedGuideLayouts.contains(locale))return;
        try {
            var field=guideme.internal.screen.GuideScreen.class.getDeclaredField("currentPage");field.setAccessible(true);
            var document=((guideme.GuidePage)field.get(screen)).document();
            int width=screen.getDocumentViewport().width(), paragraphs=0, runs=0, mandatoryExamples=0;
            var pending=new ArrayDeque<guideme.document.block.LytNode>();pending.add(document);
            while(!pending.isEmpty()) {
                var node=pending.removeFirst();pending.addAll(node.getChildren());
                if(node instanceof guideme.document.block.LytParagraph paragraph) {
                    paragraphs++;
                    for(var content:paragraph.getContent())for(var bounds:paragraph.enumerateContentBounds(content).toList()) {
                        runs++;
                        if(bounds.x()<0||bounds.right()>width)
                            throw new IllegalStateException("Guide text outside viewport: "+bounds+" width="+width);
                    }
                    String text=paragraph.getTextContent();
                    if(text.contains("get must 64")&&text.contains("wait 4 tick")) {
                        mandatoryExamples++;
                        if(paragraph.getBounds().height()>120)
                            throw new IllegalStateException("Short example expanded into excessive code lines: "+paragraph.getBounds());
                    }
                }
            }
            if(paragraphs<20||runs<100||mandatoryExamples!=1)throw new IllegalStateException("Missing rendered guide geometry");
            Files.writeString(root.getParent().resolve("guide-layout-"+locale+".json"),new com.google.gson.Gson().toJson(
                Map.of("status","passed","viewportWidth",width,"paragraphs",paragraphs,"textRuns",runs,"mandatoryExamples",mandatoryExamples)));
            checkedGuideLayouts.add(locale);
        } catch(ReflectiveOperationException|java.io.IOException failure){throw new RuntimeException(failure);}
    }
    ...
    private static FactoryBlockEntity factory(net.minecraft.server.level.ServerPlayer player, BlockPos pos) {
        return (FactoryBlockEntity) player.level().getBlockEntity(pos);
    }
    ...
    private static void setup() {
        ...
        add("minimal-cable-terminal-redstone-check",()->server(p->{
            var terminal=(FactoryBlockEntity)p.level().getBlockEntity(new BlockPos(12,101,12));
            ...
            if(!minimalNetworkRan)minimalNetworkDebug=terminal==null?"missing terminal":
                ...
                +" redstone="+p.level().hasNeighborSignal(new BlockPos(12,101,12))
        ...
        add("minimal-cable-terminal-redstone-cleanup",()->server(p->{
            for(var pos:new BlockPos[]{new BlockPos(12,101,12),new BlockPos(12,101,11),new BlockPos(12,101,13)})
                p.level().setBlockAndUpdate(pos,net.minecraft.world.level.block.Blocks.AIR.defaultBlockState());
        ...
        add("ui-code-tab-indent",()->{
            ...
            mc.screen.keyPressed(new net.minecraft.client.input.KeyEvent(org.lwjgl.glfw.GLFW.GLFW_KEY_TAB,0,0));
        },"FactoryEditorScreen",()->codeEditor().getValue().startsWith("    get")&&codeField().cursor()==4);
        add("ui-code-shift-tab-outdent",()->{
            var editor=codeEditor();editor.setFocused(true);editor.setValue("    get minecraft:item from A");
            codeField().seekCursor(net.minecraft.client.gui.components.Whence.ABSOLUTE,0);
            mc.screen.keyPressed(new net.minecraft.client.input.KeyEvent(org.lwjgl.glfw.GLFW.GLFW_KEY_TAB,0,org.lwjgl.glfw.GLFW.GLFW_MOD_SHIFT));
        ...
        add("ui-code-newline-keeps-indent",()->{
            ...
            mc.screen.keyPressed(new net.minecraft.client.input.KeyEvent(org.lwjgl.glfw.GLFW.GLFW_KEY_ENTER,0,0));
        ...
        add("ui-code-editor-inventory-key",()->mc.screen.keyPressed(new net.minecraft.client.input.KeyEvent(69,0,0)),"FactoryEditorScreen",
        ...
        add("legacy-terminal-facing-up",()->server(p->{
            ...
            p.level().setBlockAndUpdate(pos,block.defaultBlockState().setValue(property,net.minecraft.core.Direction.UP));
            if(p.level().getBlockEntity(pos)==null)throw new IllegalStateException("Legacy facing=up state could not create its block entity");
        ...
        add("ui-runtime-recursion",()->{
            server(p->p.level().setBlockAndUpdate(new BlockPos(4,101,4).north(),net.minecraft.world.level.block.Blocks.REDSTONE_BLOCK.defaultBlockState()));
        ...
        add("ui-after-runtime-error",()->{
            server(p->p.level().setBlockAndUpdate(new BlockPos(4,101,4).north(),net.minecraft.world.level.block.Blocks.AIR.defaultBlockState()));edit(VALID);
        ...
        add("ui-native-recipe-crafting",()->server(p->{
            ...
            var part=(appeng.parts.encoding.PatternEncodingTerminalPart)appeng.api.parts.PartHelper.getPart(p.level(),new BlockPos(3,100,0),net.minecraft.core.Direction.SOUTH);
        ...
        add("ui-native-recipe-slot-place",()->{
            ...
            mc.gameMode.handleContainerInput(menu.containerId,source.index,0,net.minecraft.world.inventory.ContainerInput.QUICK_MOVE,mc.player);
        ...
        add("ui-crafting-ripper",()->server(p->{
            ...
            var host=(com.example.ae2lightoptimizer.block.CraftingRipperBlockEntity)p.level().getBlockEntity(pos);
        ...
        add("ui-factory-panel-recipe",()->server(p->{
            ...
            var part=(FactoryEncodingPanel)appeng.api.parts.PartHelper.getPart(p.level(),new BlockPos(2,100,1),net.minecraft.core.Direction.SOUTH);
        ...
        add("part-factory-panel-red",()->server(p->{
            var bus=(appeng.blockentity.networking.CableBusBlockEntity)p.level().getBlockEntity(new BlockPos(2,100,1));
            ...
            var panel=(FactoryEncodingPanel)appeng.api.parts.PartHelper.getPart(p.level(),new BlockPos(2,100,1),net.minecraft.core.Direction.SOUTH);
            if(panel.getColor()!=appeng.api.util.AEColor.RED)throw new IllegalStateException("Panel did not inherit cable dye");
        }),"world",()->mc.screen==null);
        ...
        add("check-handheld-persistence",()->server(p->{
            ...
            var restored=ItemStack.CODEC.parse(p.level().registryAccess().createSerializationContext(com.mojang.serialization.JsonOps.INSTANCE),
                ItemStack.CODEC.encodeStart(p.level().registryAccess().createSerializationContext(com.mojang.serialization.JsonOps.INSTANCE),phone).getOrThrow()).getOrThrow();
            ...
            if(content==null || !FactoryPatternData.get(content.getStackInSlot(0)).code().equals(VALID))throw new IllegalStateException("Encoder pattern lost");
        ...
        add("prepare-encoder-network",()->server(p->{
            ...
            p.level().setBlockAndUpdate(new BlockPos(0,101,-2),FactoryContent.CABLE.get().defaultBlockState());
            p.level().setBlockAndUpdate(new BlockPos(1,101,-2),FactoryContent.CABLE.get().defaultBlockState());
            p.level().setBlockAndUpdate(new BlockPos(0,101,-1),net.minecraft.world.level.block.Blocks.BARREL.defaultBlockState());
            p.level().setBlockAndUpdate(new BlockPos(1,101,-1),net.minecraft.world.level.block.Blocks.BARREL.defaultBlockState());
        ...
        add("check-encoder-select-packet",()->{
            ...
            net.neoforged.neoforge.client.network.ClientPacketDistributor.sendToServer(new FactoryEncoderAction(1,BlockPos.ZERO,false,false,false));
        },"",
        ...
        add("ui-encoder-tag-highlights",()->{
            ...
            net.neoforged.neoforge.client.network.ClientPacketDistributor.sendToServer(new FactoryEncoderAction(0,new BlockPos(0,101,-1),true,true,false));
        ...
        add("ui-encoder-remove-one",()->net.neoforged.neoforge.client.network.ClientPacketDistributor.sendToServer(
        ...
        add("ui-encoder-remove-bulk",()->net.neoforged.neoforge.client.network.ClientPacketDistributor.sendToServer(
        ...
        for(var item:items) if(item instanceof BlockItem bi) {
            ...
            add("block-"+id,()->{mc.setScreen(null);server(p->{
                p.closeContainer();var level=p.level();
        ...
        for(var facing:net.minecraft.core.Direction.values()) for(boolean reverse:new boolean[]{false,true}) {
            add("arrows-"+facing.name().toLowerCase(java.util.Locale.ROOT)+(reverse?"-bottom-north-west":"-top-south-east"),()->{
                ...
                mc.setScreen(null);server(p->{var level=p.level();
        ...
        add("multi-tag-machine-groups",()->{
            ...
            server(p->{p.connection.teleport(cameraX,cameraY,cameraZ,cameraYaw,cameraPitch);MultiTagAudit.start(p.level());});
        ...
        add("complex-factory-flows",()->{
            ...
            server(p->{p.connection.teleport(cameraX,cameraY,cameraZ,cameraYaw,cameraPitch);ComplexFlowAudit.start(p.level());});
        },"world",()->ComplexFlowAudit.finished&&ComplexFlowAudit.passed);
        add("stress-world-transfers",()->server(p->StressAudit.start(p.level())),"world",()->StressAudit.finished && StressAudit.passed);
        add("topology-service-uniqueness",()->server(p->TopologyAudit.start(p.level())),"world",()->TopologyAudit.finished && TopologyAudit.passed);
        add("terminal-runtime-sfm",()->server(p->TerminalAudit.start(p.level())),"world",()->TerminalAudit.finished && TerminalAudit.passed);
        add("induction-card-cache",()->server(p->InductionAudit.start(p.level())),"world",()->InductionAudit.finished && InductionAudit.passed);
        add("compatibility-registry",()->server(p->CompatibilityAudit.run(p.level())),"world",()->CompatibilityAudit.finished && CompatibilityAudit.passed);
        if(Boolean.getBoolean("ae2lf.probe.channel"))
            add("channel-indentation-nesting",()->{
                ...
                server(p->{p.closeContainer();p.connection.teleport(cameraX,cameraY,cameraZ,cameraYaw,cameraPitch);ChannelAudit.start(p.level());});
            },"world",()->ChannelAudit.passed());
        if(Boolean.getBoolean("ae2lf.probe.recipeSet"))
            add("recipe-set-p-o",()->server(p->RecipeSetAudit.start(p.level())),"world",()->RecipeSetAudit.finished&&RecipeSetAudit.passed);
        steps.addFirst(steps.removeLast()); // Fail fast on optional storage identity mismatches.
        // Remote camera scenes run after base-grid checks so teleporting cannot unload their fixture.
        add("parallel-native-orders",()->{mc.setScreen(null);cameraX=634;cameraY=106;cameraZ=156;cameraYaw=140;cameraPitch=30;server(p->{p.connection.teleport(cameraX,cameraY,cameraZ,cameraYaw,cameraPitch);ParallelOrderAudit.start(p.level());});},"world",()->ParallelOrderAudit.finished&&ParallelOrderAudit.passed);
        add("native-transaction-capabilities",()->{mc.setScreen(null);cameraX=697;cameraY=106;cameraZ=88;cameraYaw=140;cameraPitch=30;server(p->{p.connection.teleport(cameraX,cameraY,cameraZ,cameraYaw,cameraPitch);NativeTransactionalAudit.start(p.level());});},"world",()->NativeTransactionalAudit.finished&&NativeTransactionalAudit.passed);
        server(p->{var pos=new BlockPos(0,100,-2);p.level().setBlockAndUpdate(pos,appeng.core.definitions.AEBlocks.DRIVE.block().defaultBlockState());
            ((appeng.blockentity.storage.DriveBlockEntity)p.level().getBlockEntity(pos)).getInternalInventory().setItemDirect(0,ModItems.INFINITE_LOOP_STORAGE_CELL.get().getDefaultInstance());});
        server(p->{var level=p.level();
        ...
        if(Boolean.getBoolean("ae2lf.probe.guideOnly")) {
            ...
            for(String locale:List.of("en_us","zh_cn")) {
                ...
                add("ui-guide-top-"+locale,()->{
                    ...
                    var page=net.minecraft.resources.Identifier.parse("ae2lightoptimizer:items-blocks-machines/loop_factory.md");
                ...
                for(int page=1;page<=60;page++) {
                    add("ui-guide-page-"+page+"-"+locale,()->{
                        ...
                        verifyGuideLayout(screen,locale);
        ...
        if(Boolean.getBoolean("ae2lf.probe.hugeQuantities"))
            add("huge-quantity-logistics",()->{mc.setScreen(null);cameraX=1204;cameraY=104;cameraZ=486;cameraYaw=145;cameraPitch=30;server(p->{p.closeContainer();p.connection.teleport(cameraX,cameraY,cameraZ,cameraYaw,cameraPitch);HugeQuantityAudit.start(p.level());});},"world",()->HugeQuantityAudit.finished && HugeQuantityAudit.passed);
        if(Boolean.getBoolean("ae2lf.probe.recovery"))
            add("huge-recovery-logistics",()->{mc.setScreen(null);cameraX=1284;cameraY=104;cameraZ=486;cameraYaw=145;cameraPitch=30;server(p->{p.closeContainer();p.connection.teleport(cameraX,cameraY,cameraZ,cameraYaw,cameraPitch);RecoveryAudit.start(p.level());});},"world",()->RecoveryAudit.finished && RecoveryAudit.passed);
        if(Boolean.getBoolean("ae2lf.probe.chunkLifecycle"))
            add("chunk-lifecycle-logistics",()->{mc.setScreen(null);cameraX=0;cameraY=104;cameraZ=0;cameraYaw=145;cameraPitch=30;server(p->{p.closeContainer();p.connection.teleport(cameraX,cameraY,cameraZ,cameraYaw,cameraPitch);ChunkLifecycleAudit.begin(p.level(),p);});},"world",()->ChunkLifecycleAudit.finished && ChunkLifecycleAudit.passed);
        ...
        if(Boolean.getBoolean("ae2lf.probe.nativeOnly"))steps.removeIf(step->!java.util.Set.of("native-transaction-capabilities","parallel-native-orders","compatibility-registry").contains(step.id));
    ...
    public static void tick(Minecraft client) {
        ...
        if(step.id.equals("complex-factory-flows")&&!ComplexFlowAudit.finished&&elapsed<ComplexFlowAudit.timeoutTicks()+400)return;
            if(step.id.equals("parallel-native-orders")&&!ParallelOrderAudit.finished&&elapsed<ParallelOrderAudit.timeoutTicks()+400)return;
            if(step.id.equals("native-transaction-capabilities")&&!NativeTransactionalAudit.finished&&elapsed<1400)return;
        ...
        if(step.id.equals("recipe-set-p-o") && !RecipeSetAudit.finished && elapsed<1400)return;
            ...
            if(!captured) {
                ...
                row.put("windowVisible",org.lwjgl.glfw.GLFW.glfwGetWindowAttrib(mc.getWindow().handle(),org.lwjgl.glfw.GLFW.GLFW_VISIBLE));
                if(mc.screen!=null) {
            ...
            // Fixture failure is recorded in its row; later independent fixtures must still run.
    ...
    private static void capture(String id) {
        net.minecraft.client.Screenshot.takeScreenshot(mc.getMainRenderTarget(),image->{try(image){image.writeToFile(root.resolve(id+".png"));}catch(Exception e){throw new RuntimeException(e);}});
    }
    private static final class ModelScreen extends Screen {
        ...
        @Override public void extractRenderState(net.minecraft.client.gui.GuiGraphicsExtractor g,int mx,int my,float delta) {
            ...
            g.centeredText(font,BuiltInRegistries.ITEM.getKey(stack.getItem()).toString(),width/2,18,0xffffffff);
            g.centeredText(font,stack.getHoverName(),width/2,34,0xffbcc5d5);
            g.pose().pushMatrix();g.pose().translate(width/2f-48,height/2f-48);g.pose().scale(6,6);
            g.item(stack,0,0);g.pose().popMatrix();
            g.centeredText(font,"Native Minecraft item renderer / QA view",width/2,height-24,0xffaab4c6);
        }
    }
}
