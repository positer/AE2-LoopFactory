...
public final class RecoveryAudit {
        ...
        try {
            ...
            if(phase==3) {
                ...
                if(index==1)check(!hasCargo(l.carrier),"resource_free_must_returns_plain_machine_without_phantom_budget");
                    ...
                    check(cargo.getAllKeys().equals(Set.of("resources","nativeRecovery")),"carrier_contains_no_program_routes_parameters_or_expected_debt");
    ...
    private static CompoundTag carrierCargo(ItemStack stack){var tag=(CompoundTag)stack.get(DataComponents.CUSTOM_DATA).copyTag().get("Ae2lfFactoryRecovery");check(tag.getAllKeys().equals(Set.of("version","kind","cargo")),"carrier_envelope_is_physical_only");return (CompoundTag)tag.get("cargo");}
    ...
    private static CompoundTag stateTag(FactoryBlockEntity host){var tag=new CompoundTag();host.saveAdditional(tag,level.registryAccess());return tag;}
