...
public final class CompatibilityAudit {
    ...
    public static void run(net.minecraft.server.level.ServerLevel level) {
        ...
        try {
            ...
            if(flux!=null) {
                ...
                var eu=(AEKey)Class.forName("com.glodblock.github.appflux.common.me.key.FluxKey").getMethod("of",energy).invoke(null,energy.getField("GTEU").get(null));
                var conversion=FactoryNativeTransfers.class.getDeclaredMethod("resource",AEKey.class);conversion.setAccessible(true);
                boolean distinct=conversion.invoke(null,flux)!=null&&conversion.invoke(null,eu)==null&&FactoryResourceSelector.parse("neoforge::fe").test(flux)&&!FactoryResourceSelector.parse("neoforge::fe").test(eu);
                result.put("native_FE_key_id",flux.getId().toString());result.put("native_GTEU_key_id",eu.getId().toString());
                result.put("native_FE_bridge_rejects_GTEU",distinct);
                if(!distinct)throw new IllegalStateException("Native FE bridge must never convert GTEU to FE");
            ...
            result.put("soulBlockCapability",FactoryNativeTransfers.soulCapabilityPresent());
        ...
        result.put("notCovered",List.of("Induction-card native CPU FE allocation","registered AE key types without a known sample constructor","third-party world capability handlers beyond network, cell storage, native item/fluid/energy/chemical/soul capabilities","souls and mana machine buffers are not AE key types: they transport as native capability resources and cannot enter AE storage until an addon registers a key type"));
    ...
    /** Item and fluid tags shipped by NeoForge, the game and every loaded mod, resolved through the real tag registry. */
    private static void modTags(Map<String,Object> result) {
        ...
        for(var tag:net.minecraft.core.registries.BuiltInRegistries.ITEM.getTagNames().toList()) {
            if(sampled>=12||!namespaces.contains(tag.location().getNamespace()))continue;
            var holder=net.minecraft.core.registries.BuiltInRegistries.ITEM.getTag(tag).orElse(null);
            if(holder==null||holder.size()==0)continue;
        ...
        for(var tag:net.minecraft.core.registries.BuiltInRegistries.FLUID.getTagNames().toList()) {
            if(sampled>=16||!namespaces.contains(tag.location().getNamespace()))continue;
            var holder=net.minecraft.core.registries.BuiltInRegistries.FLUID.getTag(tag).orElse(null);
            if(holder==null||holder.size()==0)continue;
    ...
    static final class KeySamples {
        ...
        static AEKey forType(String typeId) {
            return switch(typeId) {
                ...
                case "appbot:mana" -> singleton("appbot.ae2.ManaKey");
                case "arseng:source" -> singleton("gripe._90.arseng.me.key.SourceKey");
                default -> null;
            };
        }
        static AEKey singleton(String className) {
            try {return (AEKey)Class.forName(className).getField("KEY").get(null);}
            catch(ClassNotFoundException missing) {return null;}
            catch(ReflectiveOperationException failure) {throw new IllegalStateException("Unsupported optional key API: "+className,failure);}
        }
        ...
        static ItemStack cell(AEKey key) {
            String id=switch(key.getType().getId().toString()) {
                ...
                case "appbot:mana" -> "appbot:mana_cell_1k";
                case "arseng:source" -> "arseng:source_storage_cell_1k";
                default -> "";
            };
            if(!id.isEmpty()) {var item=BuiltInRegistries.ITEM.get(net.minecraft.resources.ResourceLocation.tryParse(id));
