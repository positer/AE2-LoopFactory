...
public final class FactoryPatternItem extends EncodedPatternItem<FactoryPatternDetails> {
    ...
    @Override public void appendHoverText(ItemStack stack, TooltipContext context,
            java.util.List<Component> lines, net.minecraft.world.item.TooltipFlag flags) {
        var recipe = previewRecipe(stack);
        if (recipe.isEmpty()) super.appendHoverText(stack, context, lines, flags);
        else recipe.getItem().appendHoverText(recipe, context, lines, flags);
    }
