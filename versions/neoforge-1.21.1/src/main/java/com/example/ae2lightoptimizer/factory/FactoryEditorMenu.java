...
public final class FactoryEditorMenu extends AEBaseMenu {
    ...
    public static final DeferredHolder<MenuType<?>, MenuType<FactoryEditorMenu>> TYPE = MENUS.register("factory_editor",
            () -> MenuTypeBuilder.create(FactoryEditorMenu::new, FactoryEditorHost.class)
                    .buildUnregistered(ResourceLocation.fromNamespaceAndPath(Ae2LightOptimizer.MOD_ID, "factory_editor")));
    ...
    public FactoryEditorMenu(MenuType<?> type, int id, Inventory inventory, FactoryEditorHost host) {
        ...
        registerClientAction("factoryUpload", String.class, this::upload);
        registerClientAction("factoryRecipePage", this::recipePage);
        registerClientAction("factoryNextProviders",this::nextProviders);
        registerClientAction("factoryCodeChunk", CodeChunk.class, this::acceptCodeChunk);
        if (isServerSide()) code = host.factoryDraft();
    }
    public void nextProviders() {
        if(isClientSide()){sendClientAction("factoryNextProviders");return;}
    ...
    public void upload(String targetId) {
        if(isClientSide()){sendClientAction("factoryUpload", targetId);return;}
    ...
    public void recipePage() {
        if(isClientSide()){sendClientAction("factoryRecipePage");return;}
    ...
    public void saveCode(String source) {
        if (isClientSide()) {
            ...
            for(int i=0;i<total;i++)sendClientAction("factoryCodeChunk",new CodeChunk(i,total,chunks.get(i)));
            return;
        }
