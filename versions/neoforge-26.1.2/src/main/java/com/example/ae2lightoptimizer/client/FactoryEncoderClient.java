...
public final class FactoryEncoderClient {
    ...
    private static boolean key(int key) {return GLFW.glfwGetKey(Minecraft.getInstance().getWindow().handle(),key)==GLFW.GLFW_PRESS;}
    public static void scroll(net.neoforged.neoforge.client.event.InputEvent.MouseScrollingEvent event) {
        ...
        if(event.getScrollDeltaY()!=0)net.neoforged.neoforge.client.network.ClientPacketDistributor.sendToServer(new FactoryEncoderAction(event.getScrollDeltaY()>0?-1:1,BlockPos.ZERO,false,false,hand==InteractionHand.OFF_HAND));
    }
    public static void use(net.neoforged.neoforge.client.event.InputEvent.InteractionKeyMappingTriggered event) {
        ...
        net.neoforged.neoforge.client.network.ClientPacketDistributor.sendToServer(new FactoryEncoderAction(0,hit.getBlockPos(),true,control,event.getHand()==InteractionHand.OFF_HAND,shift));
    }
    public static void render(net.neoforged.neoforge.client.event.RenderLevelStageEvent.AfterTranslucentFeatures event) {
        ...
        var pose=event.getPoseStack();var camera=event.getLevelRenderState().cameraRenderState.pos;
        var buffers=mc.renderBuffers().bufferSource();var type=net.minecraft.client.renderer.rendertype.RenderTypes.lines();var vertices=buffers.getBuffer(type);
        for(boolean selected:new boolean[]{false,true})for(long value:selected?view.selectedPositions():view.otherPositions()) {
            ...
            net.minecraft.client.renderer.ShapeRenderer.renderShape(pose,vertices,net.minecraft.world.phys.shapes.Shapes.box(-.004,-.004,-.004,1.004,1.004,1.004),pos.getX()-camera.x,pos.getY()-camera.y,pos.getZ()-camera.z,selected?0xff33ccff:0x596680ff,selected?2f:1f);
        }
        var modelView=event.getModelViewMatrix();
        var projection=event.getLevelRenderState().cameraRenderState.projectionMatrix;
    ...
    public static void renderGui(net.neoforged.neoforge.client.event.RenderGuiEvent.Post event) {
        ...
        for(var label:merged) {
            ...
            for(String tag:label.tags()) {
                ...
                graphics.text(font,tag,Math.round(label.x())-font.width(tag)/2,y,color,true);
                y+=font.lineHeight;
            }
        }
    }
