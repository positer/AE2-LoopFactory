...
public final class FactoryJob implements FactoryMachine.Host {
    ...
    @Override public long count(String tag, String resource) {
        if (tag.equals(FactoryExpression.ALL_TAGS)) {
            // A bare "has resource" reads every machine tag bound to this program.
            return countMachineTags(host.tags.names().stream().filter(FactoryJob::machineTag).toList(), resource);
        }
        if (FactoryTags.compound(tag)) return countMachineTags(FactoryTags.expression(tag), resource);
    ...
    /** Machine tags resolve to one position union so a machine listed twice is still counted once. */
    private long countMachineTags(List<String> tags, String resource) {
        var positions = new java.util.LinkedHashSet<BlockPos>();
        long extra = 0;
        for (String name : tags) {
            if (!machineTag(name)) { extra = Math.addExact(extra, count(name, resource)); continue; }
            positions.addAll(machinePositions(name));
        }
        return Math.addExact(extra, FactoryNativeTransfers.count((ServerLevel)host.getLevel(),List.copyOf(positions),nativeFilter(resource)));
    }
    ...
    private List<Target> targets(String tag, String face, boolean get) {
        if (FactoryTags.compound(tag)) {
            var all = new ArrayList<Target>();
            var seen = new java.util.LinkedHashSet<String>();
            for (String part : FactoryTags.expression(tag)) for (var target : targets(part, face, get)) {
                String key = target.position() == null ? "s:" + all.size() + ":" + part : target.position().asLong() + ":" + target.face();
                if (seen.add(key)) all.add(target);
            }
            return all;
        }
